...
#define RGB_ON 0x8000
#define CODE_CONST 0xFF00

// set rgb_on to 0x8000 if colourmode =1, otherwise set to 0x0000
static const GLchar Yglprg_vdp2_sprite_palette_rgb[] =
"uniform int rgb_on;\n"
"uniform bool code_y;\n"
"uniform int code_const;\n"
"uniform bool drawfb_calc;\n"
"uniform int drawfb_vdpmode;\n"
"uniform int ret_mask1;\n"
"uniform int ret_mask2;\n"
"uniform int retcc_shift;\n"
"uniform int retcc_mask;\n"
"uniform int retp_shift;\n"
"uniform int retp_mask;\n"
"uniform int vdp2_fb_cond;\n"
"uniform bool sprite_2; \n"
"uniform bool sprite_27; \n"
"uniform bool fb_on; \n"
"\n"
"bool isRGBCode(int index) {\n"
" return ((index & rgb_on)!=0);\n"
"}\n";
...
" return (int(col.x*255.0) | code_const);\n"
"}\n";


// use for 2+
static const GLchar Yglprg_vdp2_sprite_type_07[] =
"FBCol getVDP1PixelCode(vec2 col, bool odd) {\n"
"//Sprite type 2, 3, 4, 5, 6, 7\n"
"  FBCol ret = zeroFBCol();\n"					// same
...
"    ret.prio = 0;\n"							// same
"    ret.isRGB = 1;\n"							// same
"    if (!sprite_2) { ret.cc = 0; }\n"				// only in 0, 1, 3, 4, 5 , 6, 7 (will be incorrect for spritetype 2)
"    if ((use_sp_win != 0) && (ret.code == 0x8000)) ret.valid = 0;\n"	// only in 2, 3, 4, 5, 6, 7
"    ret.color.rgb = getRGB(ret.code).rgb;\n"		// same
"  } else {\n"								// same
"    ret.prio = (ret.code >> retp_shift) & retp_mask;\n"
"    ret.cc = (ret.code >> retcc_shift) & retcc_mask;\n"
"    if (sprite_27) {\n"
"    if (use_sp_win == 0) {\n"					// only in 2, 3, 4, 5, 6, 7
...
"    }\n"									// only in 2, 3, 4, 5, 6, 7
"    }\n"									// only in 2, 3, 4, 5, 6, 7
"    ret.code = ret.code & ret_mask1;\n"
"    ret.normalShadow = (ret.code == ret_mask2);\n"
...
static const GLchar Yglprg_vdp2_sprite_type_01[] =
"FBCol getVDP1PixelCode(vec2 col, bool odd) {\n"
"//Sprite type 0 & 1\n"
"  FBCol ret = zeroFBCol();\n"					// same
"  if (any(notEqual(col,vec2(0.0)))) ret.valid = 1;\n"	// same
...
// use for 2+
static const GLchar Yglprg_vdp2_sprite_type_27[] =
"FBCol getVDP1PixelCode(vec2 col, bool odd) {\n"
"//Sprite type 2, 3, 4, 5, 6, 7\n"
"  FBCol ret = zeroFBCol();\n"					// same
"  if (any(notEqual(col,vec2(0.0)))) ret.valid = 1;\n"	// same
...
static const GLchar Yglprg_vdp2_sprite_type_8F[] =
"FBCol getVDP1PixelCode(vec2 col, bool odd) {\n"	// same
"  //Sprite type 8, 9, A, B, C, D, E, and F\n"
"  if (odd && ((uint(col.y*255.0) & 0x80u)==0x80u)) col.x = float(uint(col.x * 255.0) | 0x80u)/255.0\n;"	// same
"  FBCol ret = zeroFBCol();\n"	// same
"  if (col.x != 0.0) ret.valid = 1;\n"	// same
"  else return ret;\n"	// same
"  ret.code = getCode(col);\n"	// same
"  ret.prio = (retp_shift < 1) ? ret.prio : ((ret.code >> retp_shift) & retp_mask);\n"
"  ret.cc = (retcc_shift < 1) ? ret.cc : ((ret.code >> retcc_shift) & retcc_mask);\n"
"  ret.code = ret.code & ret_mask1;\n"
"  ret.normalShadow = (ret.code == ret_mask2);\n"
"  ret.color.rg = getVec2(ret.code).xy;\n"			// same
"  ret.color.b = 0.0;\n"						// same
...
const GLchar * vdp2blit_srite_type_f[3] = {
//  Yglprg_vdp2_sprite_type_01,
//  Yglprg_vdp2_sprite_type_27,
  Yglprg_vdp2_sprite_type_07,
  Yglprg_vdp2_sprite_type_8F,
};

static const GLchar Yglprg_color_condition_f[] = 
"  switch (vdp2_fb_cond) { \n "
"    case 1: \n"
"	if( depth > getVDP2Reg(16, line) ) { \n fbmode = 2; \n } \n break; \n "
"    case 2: \n"
"	if( depth != getVDP2Reg(16, line) ) { \n fbmode = 2; \n } \n break; \n "
"    case 3: \n"
"	if( depth < getVDP2Reg(16, line) ) { \n fbmode = 2; \n } \n break; \n "
...
static const GLchar Yglprg_vdp2_drawfb_cram_epiloge_shared[] =
" if (fbmode == 1 && drawfb_calc) vdp1mode = drawfb_vdpmode; \n";
...
// Note that this is the same as Yglprg_vdp2_sprite_getCode_8
...
static const GLchar Yglprg_vdp2_prio_part_fb_6[] =
...
static const GLchar Yglprg_vdp2_prio_part_fb_5[] =
"Col getPriorityColor(int prio, int nbPrio)   \n"
"{  \n"
"  Col ret; \n"
"  int remPrio = nbPrio;\n"
...
static const GLchar Yglprg_vdp2_prio_part_fb_4[] =
"Col getPriorityColor(int prio, int nbPrio)   \n"
"{  \n"
"  Col ret; \n"
"  int remPrio = nbPrio;\n"
...
static const GLchar Yglprg_vdp2_prio_part_fb_3[] =
"Col getPriorityColor(int prio, int nbPrio)   \n"
"{  \n"
"  Col ret; \n"
"  int remPrio = nbPrio;\n"
...
static const GLchar Yglprg_vdp2_prio_part_fb_2[] =
"Col getPriorityColor(int prio, int nbPrio)   \n"
"{  \n"
"  Col ret; \n"
"  int remPrio = nbPrio;\n"
...
static const GLchar Yglprg_vdp2_prio_part_fb_1[] =
"Col getPriorityColor(int prio, int nbPrio)   \n"
"{  \n"
"  Col ret; \n"
"  int remPrio = nbPrio;\n"
...
static const GLchar Yglprg_vdp2_prio_part_fb_0[] =
"Col getPriorityColor(int prio, int nbPrio)   \n"
"{  \n"
"  Col ret; \n"
"  int remPrio = nbPrio;\n"
...
const GLchar * Yglprg_vdp2_common_part_screen[14] = {
  Yglprg_vdp2_prio_part_fb_0,
  Yglprg_vdp2_prio_part_fb_1,
  Yglprg_vdp2_prio_part_fb_2,
  Yglprg_vdp2_prio_part_fb_3,
  Yglprg_vdp2_prio_part_fb_4,
  Yglprg_vdp2_prio_part_fb_5,
  Yglprg_vdp2_prio_part_fb_6,
};
...
if (fb_on && pix.layer == 6) { \n \
  txcoll = getFB(-1, addr).color;\n \
  txcolll = getFB(-2, addr).color;\n \
}\n"

static const GLchar Yglprg_vdp2_common_end_6[] =
...
static const GLchar Yglprg_vdp2_common_end_5[] =
"Col getBlur(ivec2 addr, Col pix, vec2 texCoord) \n"
"{  \n"
"  Col ret = pix;\n"
"  vec4 txcoll;\n"
...
static const GLchar Yglprg_vdp2_common_end_4[] =
"Col getBlur(ivec2 addr, Col pix, vec2 texCoord) \n"
"{  \n"
"  Col ret = pix;\n"
"  vec4 txcoll;\n"
...
static const GLchar Yglprg_vdp2_common_end_3[] =
"Col getBlur(ivec2 addr, Col pix, vec2 texCoord) \n"
"{  \n"
"  Col ret = pix;\n"
"  vec4 txcoll;\n"
...
static const GLchar Yglprg_vdp2_common_end_2[] =
"Col getBlur(ivec2 addr, Col pix, vec2 texCoord) \n"
"{  \n"
"  Col ret = pix;\n"
"  vec4 txcoll;\n"
...
static const GLchar Yglprg_vdp2_common_end_1[] =
"Col getBlur(ivec2 addr, Col pix, vec2 texCoord) \n"
"{  \n"
"  Col ret = pix;\n"
"  vec4 txcoll;\n"
...
static const GLchar Yglprg_vdp2_common_end_0[] =
"Col getBlur(ivec2 addr, Col pix, vec2 texCoord) \n"
"{  \n"
"  Col ret = pix;\n"
"  vec4 txcoll;\n"
...
static const GLchar* Yglprg_vdp2_common_end[7] = {
  Yglprg_vdp2_common_end_0,
  Yglprg_vdp2_common_end_1,
  Yglprg_vdp2_common_end_2,
  Yglprg_vdp2_common_end_3,
  Yglprg_vdp2_common_end_4,
  Yglprg_vdp2_common_end_5,
  Yglprg_vdp2_common_end_6,
};
...
#define COMMON_GET_PRIORITY_SCREEN_FB_ONOFF "\
    ...
    while (hasColor != 0) {\n \
      ...
      if (prio.mode != 0) { \n \
        if (foundColor1 == 0) { \n \
          ...
          if (!fb_on || prio.isSprite == 0) {\n \
            if (fb_on && processShadow && prio.normalShadow) alpha = 0.5; \n \
            if (fb_on && (FBMesh == 1) && (i <= FBMeshPrio)) {\n \
              mesh = 1;\n \
...
//In case of a code of zero and if the transparent shadow code is enabled, then we do not process as a top image \n \
	...
	if (fb_on) { \n \
          ...
          if (fb_on) { colorsecond.rgb *= alpha; } \n \
...
#define FB_MESH "\
if (fb_on && (FBMesh == 1) && (foundColor1 == 0)) {\n \
  mesh = 1;\n \
  meshCol = FBShadow.rgb;\n \
}\n \
"
#define FB_COMPUTE_MESH "\
if (fb_on && (mesh == 0) && (FBMesh == 1)) {\n \
  meshCol = FBShadow.rgb;\n \
...
#ifdef QUICK_SHADER_TMP
static const GLchar Yglprg_vdp2_common_final_6[]=
COMMON_FINAL_START
...
COMMON_GET_PRIORITY_SCREEN_FB_ONOFF
FB_MESH
...
static const GLchar Yglprg_vdp2_common_final_5[]=
COMMON_FINAL_START
COMMON_FINAL_SCREEN(0)
COMMON_FINAL_SCREEN(1)
COMMON_FINAL_SCREEN(2)
...
COMPUTE_IMAGES
;
static const GLchar Yglprg_vdp2_common_final_4[]=
COMMON_FINAL_START
COMMON_FINAL_SCREEN(0)
COMMON_FINAL_SCREEN(1)
COMMON_FINAL_SCREEN(2)
...
COMPUTE_IMAGES
;
static const GLchar Yglprg_vdp2_common_final_3[]=
COMMON_FINAL_START
COMMON_FINAL_SCREEN(0)
COMMON_FINAL_SCREEN(1)
COMMON_FINAL_SCREEN(2)
...
COMPUTE_IMAGES
;
static const GLchar Yglprg_vdp2_common_final_2[]=
COMMON_FINAL_START
COMMON_FINAL_SCREEN(0)
COMMON_FINAL_SCREEN(1)
COMMON_GET_PRIORITY_SCREEN_FB_ONOFF
...
COMPUTE_IMAGES
;
static const GLchar Yglprg_vdp2_common_final_1[]=
COMMON_FINAL_START
COMMON_FINAL_SCREEN(0)
COMMON_GET_PRIORITY_SCREEN_FB_ONOFF
FB_MESH
...
COMPUTE_IMAGES
;
static const GLchar Yglprg_vdp2_common_final_0[]=
COMMON_FINAL_START
COMMON_GET_PRIORITY_SCREEN_FB_ONOFF
FB_MESH
COMMON_COMPUTE_SCREENS
...
static const GLchar* Yglprg_vdp2_common_final[14] = {
  Yglprg_vdp2_common_final_0,
  Yglprg_vdp2_common_final_1,
  Yglprg_vdp2_common_final_2,
  Yglprg_vdp2_common_final_3,
  Yglprg_vdp2_common_final_4,
  Yglprg_vdp2_common_final_5,
  Yglprg_vdp2_common_final_6,
};
  ...
  //VDP2 programs
        ...
        for (l = 0; l<3; l++) {
          ...
          for (m = 0; m<14; m++) {
              ...
              pYglprg_vdp2_blit_f[index][2] = Yglprg_vdp2_sprite_palette_rgb;
              pYglprg_vdp2_blit_f[index][3] = Yglprg_vdp2_sprite_getCode_16_8;
              ...
              pYglprg_vdp2_blit_f[index][7] = Yglprg_color_condition_f;
              pYglprg_vdp2_blit_f[index][8] = Yglprg_vdp2_drawfb_cram_epiloge_shared;
              ...
              pYglprg_vdp2_blit_f[index][12] = Yglprg_vdp2_common_part_screen[m % 7];
              pYglprg_vdp2_blit_f[index][13] = Yglprg_vdp2_common_end[m % 7];
...
int setupVDP2Prog(Vdp2* varVdp2Regs, int nb_screen, int CS) {
  ...
  int colormode, spritetype, screen_nb;

   //Consider x5
  if ( SPCCN ) {
    condition += ((varVdp2Regs->SPCTL >> 12) & 0x3) + 1; //5x
  }
  colormode =  (varVdp2Regs->SPCTL & 0x20) != 0; // 2x
  spritetype =  (varVdp2Regs->SPCTL & 0xF); // 16x

  screen_nb = nb_screen; //14x
...
#ifdef QUICK_SHADER
  pgid += hash_shader(mode,colormode, spritetype, screen_nb, condition, fb_mode);
#else
...
#ifdef QUICK_SHADER
  glUniform1i(glGetUniformLocation(_prgid[pgid], "rgb_on"), ((colormode == 1) ? RGB_ON : 0));
  glUniform1i(glGetUniformLocation(_prgid[pgid], "code_y"), (fb_mode == 0));
  glUniform1i(glGetUniformLocation(_prgid[pgid], "code_const"), ((fb_mode == 2) ? CODE_CONST : 0));
  glUniform1i(glGetUniformLocation(_prgid[pgid], "drawfb_calc"), (mode > 0));
  glUniform1i(glGetUniformLocation(_prgid[pgid], "drawfb_vdpmode"), (mode + 1));  
  glUniform1i(glGetUniformLocation(_prgid[pgid], "vdp2_fb_cond"), condition);
  glUniform1i(glGetUniformLocation(_prgid[pgid], "fb_on"), (screen_nb >= 7));
  glUniform1i(glGetUniformLocation(_prgid[pgid], "sprite_2"), (spritetype == 2));
  glUniform1i(glGetUniformLocation(_prgid[pgid], "sprite_27"), (spritetype >= 2));

switch (spritetype) {
	case 0:
		glUniform1i(glGetUniformLocation(_prgid[pgid], "ret_mask1"), 0x7FF);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "ret_mask2"), 0x7FE);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retcc_shift"), 11);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retcc_mask"), 0x7);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retp_shift"), 14);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retp_mask"), 0x3);
		break;
	case 1:
		glUniform1i(glGetUniformLocation(_prgid[pgid], "ret_mask1"), 0x7FF);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "ret_mask2"), 0x7FE);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retcc_shift"), 11);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retcc_mask"), 0x3);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retp_shift"), 13);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retp_mask"), 0x7);
		break;
	case 2:
		glUniform1i(glGetUniformLocation(_prgid[pgid], "ret_mask1"), 0x7FF);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "ret_mask2"), 0x7FE);
		...
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retcc_mask"), 0x7);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retp_shift"), 14);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retp_mask"), 0x1);
		break;
	case 3:
		glUniform1i(glGetUniformLocation(_prgid[pgid], "ret_mask1"), 0x7FF);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "ret_mask2"), 0x7FE);
		...
		break;
	case 4:
		glUniform1i(glGetUniformLocation(_prgid[pgid], "ret_mask1"), 0x3FF);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "ret_mask2"), 0x3FE);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retcc_shift"), 10);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retcc_mask"), 0x7);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retp_shift"), 13);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retp_mask"), 0x3);
		break;
	case 5:
		...
		glUniform1i(glGetUniformLocation(_prgid[pgid], "ret_mask2"), 0x7FE);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retcc_shift"), 11);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retcc_mask"), 0x1);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retp_shift"), 12);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retp_mask"), 0x7);
		break;
	case 6:
		glUniform1i(glGetUniformLocation(_prgid[pgid], "ret_mask1"), 0x3FF);
		...
		break;
	case 7:
		glUniform1i(glGetUniformLocation(_prgid[pgid], "ret_mask1"), 0x1FF);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "ret_mask2"), 0x1FE);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retcc_shift"), 9);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retcc_mask"), 0x7);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retp_shift"), 12);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retp_mask"), 0x7);
		break;
	case 0x8:
		glUniform1i(glGetUniformLocation(_prgid[pgid], "ret_mask1"), 0x7F);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "ret_mask2"), 0x7E);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retcc_shift"), 6);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retcc_mask"), 0);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retp_shift"), 7);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retp_mask"), 1);
		break;
	case 0x9:
		glUniform1i(glGetUniformLocation(_prgid[pgid], "ret_mask1"), 0x3F);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "ret_mask2"), 0x3E);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retcc_shift"), 6);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retcc_mask"), 0x1);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retp_shift"), 7);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retp_mask"), 1);
	...
	case 0xA:
		...
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retcc_shift"), 6);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retcc_mask"), 0);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retp_shift"), 6);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retp_mask"), 3);
		break;
	case 0xB:
		glUniform1i(glGetUniformLocation(_prgid[pgid], "ret_mask1"), 0x3F);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "ret_mask2"), 0x3E);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retcc_shift"), 6);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retcc_mask"), 0x3);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retp_shift"), 0);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retp_mask"), 0);
		break;
	case 0xC:
		glUniform1i(glGetUniformLocation(_prgid[pgid], "ret_mask1"), 0xFF);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "ret_mask2"), 0xFE);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retcc_shift"), 6);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retcc_mask"), 0);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retp_shift"), 7);
		glUniform1i(glGetUniformLocation(_prgid[pgid], "retp_mask"), 1);
...
GLuint createCSProgram(int id, int count, const GLchar * cs[]) {
  ...
  GLint status;
    ...
    YGLLOG("Create CS Program %d..\n", id);
  _prgid[id] = glCreateProgram();
   if (_prgid[id] == 0 ) return -1;

  result = glCreateShader(GL_COMPUTE_SHADER);
